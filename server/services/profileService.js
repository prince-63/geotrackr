...
export const profilePictureUpload = async (req, res) => {
    try {
        ...
        return responseHandler(res, 200, 'success', 'Profile picture uploaded successfully', {
            profilePictureUrl: response.secure_url,
        });
    } catch (error) {
        console.error('Error occurred:', error);
        return errorResponseHandler(res, 500, 'fail', 'An error occurred while uploading the profile picture', error);
    }
    finally {
...
export const profileDetailsUpload = async (req, res) => {
    ...
    try {
        ...
        if (!contactNumber) {
            return errorResponseHandler(res, 400, 'fail', 'Contact number is required');
        }
        if (!dateOfBirth) {
            return errorResponseHandler(res, 400, 'fail', 'Date of birth is required');
        }
        if (!bloodGroup) {
        ...
        if (!organizationName) {
            return errorResponseHandler(res, 400, 'fail', 'Organization name is required');
        }
        if (!organizationAddress) {
            return errorResponseHandler(res, 400, 'fail', 'Organization address is required');
        }
        if (!organizationContactNumber) {
            return errorResponseHandler(res, 400, 'fail', 'Organization contact number is required');
        }
        ...
        return responseHandler(res, 200, 'success', 'Profile updated successfully', updateUser);
    } catch (error) {
        console.log(error);
...
export const getProfileDetails = async (req, res) => {
    ...
    try {
        ...
        return responseHandler(res, 200, 'success', 'User details fetched successfully', userDetails);
    } catch (error) {
        console.log(error);
        return errorResponseHandler(res, 500, 'fail', 'Internal server error');
    } finally {
