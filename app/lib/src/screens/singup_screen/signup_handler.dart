import 'dart:convert';

import 'package:app/config/api_config.dart';
import 'package:app/src/helper/error_handler.dart';
import 'package:app/src/utils/share_preferences.dart';
import 'package:app/src/utils/uri_parser.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class SignupHandler {
  static void signup(String username, String email, String password, BuildContext context) async {
    final url = UriParser.parse(ApiConfig.authSignup);

    if (username.isEmpty) {
      ErrorHandler.displayError(context, "Please enter a valid username");
      return;
    }

    if (email.isEmpty) {
      ErrorHandler.displayError(context, "Please enter a valid email");
      return;
    }

    if (password.isEmpty) {
      ErrorHandler.displayError(context, "Please enter a valid password");
      return;
    }

    await _postRequest(url, {'username': username, 'email': email, 'password': password}, context);
  }

  static Future<void> _postRequest(
      Uri url, Map<String, String> body, BuildContext context) async {
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(body),
      );

      if (response.statusCode == 200) {
        final responseBody = jsonDecode(response.body);

        final token = responseBody['data']['token'];
        final email = responseBody['data']['email'];

        if (token != null && email != null) {
          SharePreferences.saveString('token', token);
          SharePreferences.saveString('email', email);
          Navigator.pushNamed(context, '/login');
        } else {
          ErrorHandler.displayError(
              context, "Signup failed: token or email is null");
          throw Exception('Signup failed: token or email is null');
        }
      } else {
        ErrorHandler.displayError(context,
            "Failed to signup: ${response.statusCode} ${response.reasonPhrase}");
        throw Exception(
            'Failed to signup: ${response.statusCode} ${response.reasonPhrase}');
      }
    } catch (e) {
      ErrorHandler.displayError(context, "Signup Failed: $e");
      throw Exception('Signup Failed: $e');
    }
  }
}